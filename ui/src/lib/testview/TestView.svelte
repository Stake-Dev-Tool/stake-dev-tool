<script lang="ts">
  ...
  let { ctx }: { ctx: TestViewContext } = $props();

  // Every devtool HTTP/SSE/prepare call is bound to the injected apiBase
  // (`''` on desktop → same-origin relative paths, byte-identical to before).
  // Aliased back to the original client names so the call sites are unchanged.
  // `ctx` is injected once per mount and never changes; `untrack` reads its
  // initial value without a (false-positive) reactivity subscription.
  const http = untrack(() => createDevtoolClient(ctx.apiBase));
  const {
    settings: settingsHttp,
    forcedEvent: forcedEventHttp,
    savedRounds: savedRoundsHttp,
    sessions: sessionsHttp,
  ...
  $effect(() => {
    ...
    for (const f of frames) {
      ...
      const frame = f;
      const es = new EventSource(http.streamUrl(frame.sessionId));
  ...
  function defaultSessionIdFor(resolutionId: string): string {
    return ctx.makeSessionId(resolutionId);
  }
    ...
    {:else}
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-2.5">
          <!-- Force event -->
          {#if ctx.capabilities.forceEvent}
          ...
          <!-- Saved rounds -->
          {#if ctx.capabilities.saveRounds}
      ...
      <!-- ========== LAYOUT ========== -->
      {#if ctx.capabilities.manageResolutions}
      <Card.Root>
        <Card.Header class="pb-3">
          <Card.Title class="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
            Layout
          ...
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-1">
              <Tooltip.Provider delayDuration={300}>
                ...
                {#if ctx.capabilities.openInBrowser}
                <Tooltip.Root>
                  <Tooltip.Trigger>
                    {#snippet child({ props })}
                      <Button
